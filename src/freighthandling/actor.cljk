(ns freighthandling.actor
  "FreightHandlingPracticeActor — the ISCO-08 9333 community freighthandling actor as a
  `langgraph.graph/state-graph` (ADR-2607011000 / CLAUDE.md Actors
  section). One graph run = one freighthandling operation request
  ...
  The unconditional invariant: the FreightHandlingPracticeAdvisor can never
  directly commit a record the FreightHandlingPracticeGovernor refuses —
  every commit-record! call is gated behind `:decide`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [freighthandling.advisor :as advisor]
            [freighthandling.governor :as governor]
            [freighthandling.store :as store]))

(defn build-graph
  "Build a compiled FreightHandlingPracticeActor graph. `store` implements
  `freighthandling.store/Store`. `advisor` implements
  `freighthandling.advisor/Advisor` (defaults to `mock-advisor`).
      ...
      (g/add-node :commit
                   (fn [{:keys [request proposal]}]
                     (let [record {:client-id (:client-id request)
                                    :op (:op proposal)
                                    :shipment-id (:shipment-id proposal)
                                    :payload proposal}]
